...
namespace PJMT.Framework.DependencyInjection.Interfaces
{
    ...
    public static class ActivatorUtilities
    {
        ...
        public static object CreateInstance(IServiceProvider provider, Type instanceType, params object[] parameters)
        {
            ...
            if (bestMatcher == null)
            {
                throw new InvalidOperationException(
                    string.Format(Resources.NoConstructorMatch, instanceType)
                );
            }
        ...
        private static object GetService(IServiceProvider sp, Type type, Type requiredBy, bool isDefaultParameterRequired)
        {
            ...
            {
                throw new InvalidOperationException(
                    string.Format(Resources.CannotResolveService, type, requiredBy)
                );
            }
            return service;
        }
            ...
            out int?[] parameterMap)
        {
            ...
            foreach (var constructor in instanceType.GetTypeInfo().DeclaredConstructors)
            {
                ...
                if (TryCreateParameterMap(constructor.GetParameters(), argumentTypes, out tempParameterMap))
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            string.Format(Resources.AmbiguousConstructorMatch, instanceType)
                        );
                    }
                ...
                for (var index = 0; index != _parameters.Length; ++index)
                {
                    if (_parameterValuesSet[index] == false)
                    {
                        ...
                        if (value == null)
                        {
                            ...
                            {
                                throw new InvalidOperationException(
                                    string.Format(Resources.CannotResolveService, _parameters[index].ParameterType, _constructor.DeclaringType)
                                );
                            }
                            else
                            {
